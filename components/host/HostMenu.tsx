...
const HostMenu: React.FC<HostMenuProps> = ({ properties, onNavigate }) => {
  ...
  const fetchTasks = async () => {
    ...
    if (error) {
      console.error('--- SUPABASE ERROR [fetchTasks] ---', error);
      if (error.code === '42P01') {
        setDbError('Tabla "tasks" no existe en la base de datos.');
      }
    } else {
      setTasks(data || []);
    }
  ...
  const fetchEarnings = async () => {
    ...
    if (error) {
      console.error('--- SUPABASE ERROR [fetchEarnings] ---', error);
      if (error.code === '42P01') { // Code for "relation does not exist"
        setDbError('Faltan tablas en Supabase. Requiere ejecución de schema SQL.');
      } else {
        setDbError(`Error de conexión Supabase: ${error.message || 'Error desconocido'}`);
      }
    } else {
      setDbError(null);
      setEarnings(data || []);
