...
export default async function handler(req, res) {
    ...
    try {
        ...
        for (const prop of properties) {
            ...
            for (const feed of feeds) {
                ...
                try {
                    ...
                    if (!response.ok)
                        throw new Error(`HTTP ${response.status} from ${feed.platform}`);
                    ...
                    if (blocks.length > 0) {
                        ...
                        if (upsError)
                            throw new Error(`Atomic Upsert failed: ${upsError.message}`);
                        ...
                        if (purgeError)
                            console.warn(`[Sync-iCal] Non-critical purge failure: ${purgeError.message}`);
        ...
        if (logErr)
            console.error('[Sync-iCal] Failed to log heartbeat:', logErr.message);
...
//# sourceMappingURL=sync-ical.js.map