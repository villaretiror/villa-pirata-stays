...
function checkIcsOverlap(icsText, checkIn, checkOut) {
    ...
    for (const rawLine of lines) {
        ...
        if (line === 'END:VEVENT' && inEvent) {
            ...
            const parseToYMD = (raw) => {
                ...
                if (rawD.length >= 8)
                    return Number(`${rawD.substring(0, 4)}${rawD.substring(4, 6)}${rawD.substring(6, 8)}`);
                return 0;
            };
            const blockS = parseToYMD(dtStart);
            const blockE = parseToYMD(dtEnd);
            if (blockS && blockE) {
                if (reqStart < blockE && blockS < reqEnd)
                    return true; // CRITICAL: Ghost Overlap Detected
            }
        }
        if (inEvent) {
    ...
    try {
        ...
        if (booking.property_id && booking.check_in && booking.check_out) {
            ...
            if (prop && prop.calendarSync && Array.isArray(prop.calendarSync)) {
                const validFeeds = prop.calendarSync.filter((feed) => !!feed.url);
                // 🔥 PARALLEL EXECUTION (ANTI-TIMEOUT)
                const validations = await Promise.allSettled(validFeeds.map(async (feed) => {
