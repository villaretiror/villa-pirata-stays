    ...
    try {
        ...
        switch (type) {
            ...
            case 'reservation_confirmed': {
                ...
                await NotificationService.notifyNewReservation(rest.bookingId || 'ID_N/A', firstName, p.name, rest.checkIn || 'Fecha', rest.checkOut || 'Fecha', rest.total || '0', 'Web Directa');
                break;
            }
            case 'lead_recovery': {
        ...
        for (const options of emailOptions) {
            ...
            if (data?.id) {
                await supabase.from('email_logs').insert({
                    resend_id: data.id,
                    booking_id: rest.bookingId || null,
