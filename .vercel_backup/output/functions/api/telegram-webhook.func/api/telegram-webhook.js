...
const supabaseServiceRole = createClient(process.env.SUPABASE_URL || process.env.VITE_SUPABASE_URL || "", process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.VITE_SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_ANON_KEY || process.env.VITE_SUPABASE_ANON_KEY || "");
export default async function handler(req, res) {
    ...
    try {
        ...
        async function notifyHost(message) {
...
async function handleAIConsultation(chatId, text, from, imagePart) {
...
`.trim();
        ...
        const initialParts = [{ text: text || "Analiza esta situación del negocio." }];
        if (imagePart)
            initialParts.push(imagePart);
        // 🔄 ROBUST MESSAGE HISTORY (contents)
        let contents = [{ role: 'user', parts: initialParts }];
        let finalResponse = "";
        let iterations = 0;
        while (iterations < 5) {
