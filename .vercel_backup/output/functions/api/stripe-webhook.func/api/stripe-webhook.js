                ...
                Promise.allSettled([
                    ...
                    results.forEach((r, i) => {
                        ...
                        if (r.status === 'rejected')
                            console.error(`[Stripe Webhook] ${labels[i]} failed:`, r.reason);
                        else
                            console.log(`[Stripe Webhook] ${labels[i]}: ${r.value ? '✅' : '❌'}`);
                    });
                });
            }
            else {
...
// ============================================================
async function fireOperationsManifesto(bookingId, booking) {
    try {
        ...
        if (booking?.addons_breakdown) {
            const addonsList = Array.isArray(booking.addons_breakdown)
    ...
    `;
        ...
        if (data.id) {
            supabase.from('email_logs').insert({
                ...
                status: 'sent'
            }).then(null, (e) => console.error('[OperationsManifesto] DB Log Error:', e.message));
            return true;
        }
...
// ============================================================
async function fireCaptainTelegramAlert(bookingId, booking) {
    try {
        ...
        if (booking?.addons_breakdown) {
            const addonsList = Array.isArray(booking.addons_breakdown)
                ...
                : Object.keys(booking.addons_breakdown).filter((k) => booking.addons_breakdown[k]);
            if (addonsList.length > 0) {
                addonsLine = addonsList.map((a) => `<b>${addonLabels[a] || `✨ ${a}`}</b>`).join('\n');
            }
        }
...
// ============================================================
async function fireGuestConfirmationEmail(bookingId, booking) {
    try {
        const siteUrl = getEnvVar('VITE_SITE_URL') || 'https://www.villaretiror.com';
