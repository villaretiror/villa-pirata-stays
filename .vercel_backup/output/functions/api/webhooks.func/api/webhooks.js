            ...
            if (type === 'end-of-call-report') {
                ...
                if (report.recordingUrl) {
                    try {
                        ...
                        if (pubData)
                            localRecordingUrl = pubData.publicUrl;
                    }
                    catch (e) {
 ...
 */
async function handleVapiTools(req, res, message) {
 ...
 */
async function executeDirectTool(args, supabase) {
    ...
    const { start: finalStartDate, end: finalEndDate, isNormalized } = (() => {
        ...
        try {
            ...
            if (changed)
                console.log(`[🔱 Chronos] Normalized: ${finalStart} -> ${finalEnd} (${nights} nights)`);
    ...
    const resolvePropIdAntiHallucination = async (rawId, rawName, sb) => {
        ...
        if (cleanName.includes('retiro') || cleanName.includes('retiro r') || cleanName === 'villa retiro r') {
            if (cleanId !== '1081171030449673920')
                console.log(`[🔱 Resolution] Resolved propertyId from propertyName fallback: Villa Retiro R`);
            return '1081171030449673920';
        }
        if (cleanName.includes('pirata') || cleanName.includes('family') || cleanName.includes('house')) {
            if (cleanId !== '42839458')
                console.log(`[🔱 Resolution] Resolved propertyId from propertyName fallback: Pirata Family House`);
            return '42839458';
        }
        ...
        if (finalCheck)
            return String(finalCheck.id);
        return null; // Unresolvable
    };
    try {
        let toolName = name;
        if (!toolName) {
            if (startDate || args.check_in)
                toolName = 'check_availability';
            else if (phone || args.telefono)
                toolName = 'send_payment_sms';
        }
        ...
        if (toolName === 'check_availability') {
            ...
            if (!phone)
                throw new Error("Missing phone for SMS.");
            ...
            if (finalStartDate && finalEndDate) {
                try {
                    ...
                    console.warn("[Webhook] Price recalculation failed, using AI fallback.");
                }
            }
            ...
            if (finalStartDate && finalEndDate) {
                try {
                    ...
                    console.warn("[Webhook] Price recalculation failed for email, using AI fallback.");
                }
            }
