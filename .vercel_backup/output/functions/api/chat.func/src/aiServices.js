...
const GoogleGenAIClass = GoogleGenAIModule.GoogleGenAI || GoogleGenAIModule.default || GoogleGenAIModule;
 ...
 */
export const queryPropertyKnowledge = async (query, rawPropertyId, client, options = {}) => {
    ...
    // B) Amenities: What includes? Pool? WiFi?
        ...
        const specificSearch = (prop.amenities || []).find((a) => q.includes(a.toLowerCase()));
        if (specificSearch) {
            answer = `Sí, la propiedad cuenta con ${specificSearch}. Además de otras amenidades principales como ${amenitiesList.filter((a) => a !== specificSearch).slice(0, 5).join(', ')}.`;
        }
...
export const resolvePropertyId = async (input, client) => {
    let cleanInput = String(input).trim();
    // 1. Direct check (If it's a valid ID)
    if (!isNaN(Number(cleanInput)) && cleanInput.length >= 8)
        return cleanInput;
    if (cleanInput.length > 20)
        return cleanInput; // UUID-like
    // 🔱 PRIORITY SHORT-MAP: Instant recognition for keywords (Ultra-Tolerant)
    const lower = cleanInput.toLowerCase();
    if (lower.includes('retiro') || lower.includes('villa') || lower.includes('retiro r'))
        return '1081171030449673920';
    if (lower.includes('pirata') || lower.includes('family') || lower.includes('house'))
        return '42839458';
...
export const checkAvailabilityWithICal = async (villaId, checkIn, checkOut, customSupabase) => {
    ...
    const nativeOverlap = (bookingRes.data || []).find((b) => {
        // 🛡️ INVENTORY PROTECTION: Only block for real intention
        if (!BLOCKING_STATUSES.includes(b.status || ''))
            return false;
        // AI Hold TTL check
        if (b.status === 'pending_ai_validation' && b.hold_expires_at && new Date(b.hold_expires_at) < now)
            return false;
    ...
    const externalOverlap = (syncedRes.data || []).find((b) => {
        const bIn = new Date(b.check_in);
        const bOut = new Date(b.check_out);
        return qIn < bOut && qOut > bIn;
    });
    ...
    const hasPendingOverlap = (dbPending || []).some((p) => {
    ...
    if (rules && rules.length > 0) {
        ...
        while (curr < qOut) {
            ...
            const isBlocked = rules.some((r) => ds >= r.start_date && ds <= r.end_date);
            if (isBlocked) {
 ...
 */
export const findNextAvailability = async (propertyId, afterDate, nights, customSupabase) => {
    const client = customSupabase || supabase;
    ...
    const slots = Array.isArray(gaps) ? gaps : gaps.slots || [];
    const validGap = slots.find((g) => g.nights >= minNights);
    if (!validGap) {
        return {
            ok: true,
            found: false,
 ...
 */
export const findAlternatePropertyAvailable = async (excludedVillaId, checkIn, checkOut, customSupabase) => {
    const client = customSupabase || supabase;
    ...
    const availabilityChecks = await Promise.all(others.map(async (p) => {
...
export const logAbandonmentLead = async (data) => {
...
export const createTemporaryHold = async (propertyId, checkIn, checkOut, userId, customer_name, phone_number, special_requests) => {
    // 🛡️ REINFORCED RESOLUTION: Centralized Power
...
export const getPaymentVerificationStatus = async (bookingId) => {
    const { data: booking } = await supabase.from('bookings').select('payment_proof_url, status').eq('id', bookingId).maybeSingle();
    if (!booking)
        return 'Reserva no encontrada.';
    if (booking.status === 'confirmed')
        return 'El pago ya ha sido verificado y confirmado.';
    if (booking.payment_proof_url)
        return 'He recibido tu imagen de comprobante. El Host la validará en breve para confirmar tu estancia.';
    return 'Aún no hemos recibido el comprobante de pago.';
};
// 4. Gap Automation (Revenue Optimization & Sentinel Vision)
export const findCalendarGaps = async (propertyId, customSupabase) => {
    const client = customSupabase || supabase;
    const now = new Date();
    ...
    for (const b of realBookings) {
        ...
        if (gaps.length >= 5)
            break; // Optimization: First 5 slots are enough for Salty
    }
...
export const handleCrisisAlert = async (full_name, message, contact, severity = 1) => {
...
export const checkUserConcessions = async (userId) => {
    const { data: profile } = await supabase.from('profiles').select('given_concessions').eq('id', userId).single();
    if (!profile || !profile.given_concessions)
        return { allowed: true };
    const concessions = profile.given_concessions;
    ...
    if (recentConcession)
        return { allowed: false, lastGrant: recentConcession.date };
    return { allowed: true };
};
export const applyAIQuote = async (propertyId, checkIn, checkOut, promoCode, customSupabase) => {
    const client = customSupabase || supabase;
    // 🛡️ REINFORCED RESOLUTION: Centralized Power
    const finalId = await resolvePropertyId(propertyId, client);
    ...
    try {
        ...
        const quote = FinanceService.calculateReservation({
            property: property, // Cast to any to handle Omit types vs database types
            startDate: sDate,
            endDate: eDate,
            promo: promo
        });
...
export const generateOnboardingDraft = async (stage, guestName, propertyTitle, checkOutDate) => {
    let mission = "";
    if (stage === 'check_in') {
