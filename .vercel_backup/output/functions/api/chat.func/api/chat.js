...
import { checkAvailabilityWithICal, findCalendarGaps, resolvePropertyId, } from '../src/aiServices.js';
export const config = {
    runtime: 'edge',
};
const supabase = createClient(process.env.SUPABASE_URL || process.env.VITE_SUPABASE_URL || "", process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.VITE_SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_ANON_KEY || process.env.VITE_SUPABASE_ANON_KEY || "");
...
export default async function handler(req) {
    ...
    (async () => {
        try {
            ...
            (dbProperties || []).forEach((p) => { propertyTitles[p.id] = p.title; });
            ...
            const hasAudio = rawMessages.some((m) => Array.isArray(m.content) && m.content.some((c) => c.inlineData));
...
`.trim();
            ...
            const toolExecutors = {
                check_availability: async (args) => {
                    ...
                    const results = await Promise.all(villa_ids.map((id) => checkAvailabilityWithICal(id, cin, cout, supabase)));
