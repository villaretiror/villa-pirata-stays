...
export default async function handler(req, res) {
    ...
    if (req.method === 'POST' && action === 'notify') {
        ...
        try {
            ...
            else if (type === 'cohost_action')
                sent = await NotificationService.notifyCohostAction(req.body.cohost, req.body.property, req.body.action);
    ...
    if (req.method === 'GET' || (req.method === 'POST' && task)) {
        const results = { timestamp: new Date().toISOString(), summary: {} };
            ...
            taskPromises.push(runTask('automation', async () => {
                ...
                if (checkinSoon) {
                    for (const b of checkinSoon) {
                        ...
                        if (p) {
                            ...
                            await supabase.from('bookings').update({ instructions_sent_at: new Date().toISOString() }).eq('id', b.id);
                ...
                if (abandonedLeads) {
                    for (const lead of abandonedLeads) {
                        const propId = lead.tags?.find((t) => propertyMap[t]) || properties?.[0]?.id || '';
                        ...
                        await supabase.from('leads').update({ status: 'recovered' }).eq('id', lead.id);
        ...
        if (task === 'weekly_report') {
            await runTask('weekly_report', async () => {
                ...
                let rulesStr = activeRules?.length ? activeRules.map((r) => `• ${pMap(r.property_id)}: ${r.start_date} al ${r.end_date} ${r.min_nights ? `[Min ${r.min_nights}N]` : ''}`).join('\n') : "No hay reglas especiales activas.";
                await NotificationService.sendTelegramAlert(`📊 <b>ESTRATEGIA SEMANAL</b>\n\n${rulesStr}`);
                return { rules_count: activeRules?.length || 0 };
            });
        }
        // Helper for summary
        function pMap(id) { return propertyMap[id]?.title || id; }
        // --- Home Health Report ---
