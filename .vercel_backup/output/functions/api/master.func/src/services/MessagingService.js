...
export const MessagingService = {
    ...
    async sendReservationConfirmation(params) {
        ...
        const { data: p } = await supabase
            ...
            .single();
        if (!p)
            throw new Error("Property not found for messaging");
        ...
        // 2. Notify Telegram
        await NotificationService.notifyNewReservation(params.bookingId, params.guestName, p.title, params.checkIn, params.checkOut, params.total, 'Web Directa');
    },
    /**
    ...
    async sendSms(options) {
        ...
        try {
            ...
            const client = twilio(process.env.TWILIO_ACCOUNT_SID || process.env.VITE_TWILIO_ACCOUNT_SID, process.env.TWILIO_AUTH_TOKEN || process.env.VITE_TWILIO_AUTH_TOKEN);
    ...
    async sendPaymentLinkEmail(params) {
        const { data: p } = await supabase
            ...
            .single();
        if (!p)
            throw new Error("Property not found for email link");
