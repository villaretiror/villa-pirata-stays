...
export const CalendarSyncService = {
    ...
    parseIcsToBlocks(icsText, propertyId) {
        ...
        for (const rawLine of lines) {
            ...
            if (inEvent) {
                ...
                if (line.startsWith('SUMMARY'))
                    summary = (line.split(':').pop() || '').trim();
                if (line.startsWith('UID'))
                    uid = (line.split(':').pop() || '').trim();
            }
        }
        return blocks;
    },
    async syncBlocksWithDb(supabase, propertyId, propertyTitle, blocks, platform) {
        let insertedCount = 0;
        let deletedCount = 0;
        let updatedCount = 0;
        const { data: existing } = await supabase
            ...
            .eq('source', platform);
        const existingMap = new Map((existing || []).map((b) => [`${b.check_in}_${b.check_out}`, b]));
        const incomingKeys = new Set(blocks.map(b => `${b.start}_${b.end}`));
        const idsToDelete = (existing || [])
            .filter((b) => !incomingKeys.has(`${b.check_in}_${b.check_out}`))
            .map((b) => b.id);
    ...
    async getBlockedDatesFromUrl(url) {
        try {
