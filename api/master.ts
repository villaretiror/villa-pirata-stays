...
export default async function handler(req: any, res: any) {
    try {
        ...
        if (!SUPABASE_URL || !SUPABASE_SERVICE_KEY) {
            throw new Error("Missing Supabase configuration (URL or Service Key). Check environment variables.");
        }
        ...
        if (req.method === 'GET' || (req.method === 'POST' && task)) {
                ...
                taskPromises.push(runTask('automation', async () => {
                    ...
                    if (guestsCheckedOut) {
                        for (const b of guestsCheckedOut) {
                            ...
                            }).catch(e => console.error("[Automation] Review request failed:", e.message));
                    ...
                    if (abandonedLeads) {
                        for (const lead of abandonedLeads) {
                            ...
                            }).catch(e => console.error("[Automation] Recovery email failed:", e.message));
            ...
            if (task === 'health') {
                const healthRes = await runTask('health', async () => {
                    ...
                    // FIXED: timestamp instead of created_at
                    const { data: recentSyncs } = await supabase.from('cron_heartbeats').select('*').order('timestamp', { ascending: false }).limit(5);
                    results_health.sync_history = recentSyncs;
                    return results_health;
                });
                return res.status(200).json(healthRes);
            }

            // E. Telegram Life Signal (Test Bot)
            if (task === 'test_bot') {
                const testRes = await runTask('test_bot', async () => {
            ...
            if (task === 'weekly_report') {
                await runTask('weekly_report', async () => {
                    ...
                    const pMap = (id: string) => propertyMap[id]?.title || id;
            ...
            try {
                ...
                const hour = parseInt(prTime);
                if (hour === 8 || req.query?.force === 'true') {
                ...
                console.error("[Cron Health Report Error]:", e.message);
            }
        ...
        console.error("[CRON MASTER FATAL ERROR]:", globalError.message);
        return res.status(500).json({ error: 'Critical Internal Failure', message: globalError.message });
    }
}
