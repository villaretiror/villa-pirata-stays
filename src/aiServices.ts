...
export const queryPropertyKnowledge = async (
    ...
    // G) Specialized Knowledge: Solar, Payments, Location Secrets (VILLA_KNOWLEDGE Sync)
        ...
        if (q.includes('solar') || q.includes('placa') || q.includes('energía')) {
            answer = `Contamos con un sistema solar industrial de respaldo para su confort. En caso de interrupción del servicio eléctrico externo (de lo cual no somos responsables), la duración de esta reserva solar depende DIRECTAMENTE del uso responsable que usted haga del sistema. Es indispensable su cooperación moderando el uso de equipos de alto consumo (calentadores, microondas o aires acondicionados prolongados) para asegurar que la carga rinda al máximo y evitar interrupciones por agotamiento de baterías. Su colaboración es clave para mantener su estancia con energía.`;
